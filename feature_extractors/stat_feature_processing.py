...
city_data = pd.read_csv('feature_extractors/data/city.csv')
...
def calculate_statistics(dataframe):
    dataframe['population_per_house_1000'] = dataframe['reform_house_population_1000'] / dataframe[
        'reform_count_of_houses_1000']
    dataframe['population_per_house_500'] = dataframe['reform_house_population_500'] / dataframe[
        'reform_count_of_houses_500']

    dataframe['population_per_floor_1000'] = dataframe['population_per_house_1000'] / dataframe[
        'reform_mean_floor_count_1000']
    dataframe['population_per_floor_500'] = dataframe['population_per_house_500'] / dataframe[
...
def add_additional_data(dataframe):
    ...
    dataframe['distance_from_centre_in_km'] = dataframe.apply(
        lambda x: haversine(x.lng, x.lat, x.centre_lng, x.centre_lat), axis=1)
    ...
    dataframe['population'] = dataframe.apply(
        lambda x: x['osm_city_nearest_population'] if x['is_nearest_city'] else x['population'], axis=1)
...
def global_datarfame_statistics(dataframe, path_to_train, is_train=False):
    if is_train:
        lower500k = dataframe[(dataframe['population'] < 500000)][
            ["population", "realty_type", "per_square_meter_price"]].groupby("realty_type").median()
        higher500k_lower_1kk = dataframe[(dataframe['population'] > 500000) & (dataframe['population'] < 1000000)][
            ["population", "realty_type", "per_square_meter_price"]].groupby("realty_type").median()
        higger_1kk = dataframe[dataframe['population'] > 1000000][
            ["population", "realty_type", "per_square_meter_price"]].groupby("realty_type").median()

        region_stats = dataframe[["federal_district", "realty_type", "per_square_meter_price"]].groupby(
            ["federal_district", 'realty_type']).median()
        ...
        lower500k = train[(train['population'] <= 500000)][
            ["population", "realty_type", "per_square_meter_price"]].groupby("realty_type").median()
        higher500k_lower_1kk = train[(train['population'] > 500000) & (train['population'] < 1000000)][
            ["population", "realty_type", "per_square_meter_price"]].groupby("realty_type").median()
        higger_1kk = train[train['population'] >= 1000000][
            ["population", "realty_type", "per_square_meter_price"]].groupby("realty_type").median()

        region_stats = train[["federal_district", "realty_type", "per_square_meter_price"]].groupby(
            ["federal_district", 'realty_type']).median()

    dataframe['mean_price_by_type_and_population'] = dataframe.apply(
        lambda x: get_mean(x, lower500k, higher500k_lower_1kk, higger_1kk), axis=1)
